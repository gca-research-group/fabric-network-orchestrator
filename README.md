...
For `generate`, the `--seed` flag is required and selects the valid YAML configuration to mutate. Existing seeds such as `samples/network-with-chaincode.yml` can also be used. Use `--mutation-count` to change the number of mutations per scenario (the default is `3`) and `--output` to select the generated corpus directory (the default is `output`). Generation writes mutated configurations to `<output-directory>/config/` and the scenario manifest to `<output-directory>/scenarios.json`. Each manifest entry has a `mutations` array whose objects contain the expected `rule` and the zero-based `operatorIndex` within that rule's operator group, allowing the exact mutation to be selected again. Older `rules`-only manifests are not accepted. Generation does not run verification or create or update `results.json`.

`validate` reads the existing manifest and scenario files from `--output` (default `output`) and writes `<output-directory>/results.json`. The result document contains only its `results` array; aggregate counts are recorded in validation metadata. It needs neither the original seed nor a mutation count and does not regenerate scenarios. Each result is `passed` when all expected rules are reported, `partial` when only some are reported, or `failed` when none are reported or the configuration cannot be processed. Missing rules are included in the result. Validation exits with a non-zero status when any scenario is partial or failed. Generation retains its existing corpus overwrite behavior; validation replaces existing results.
