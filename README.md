...
- `runner` streams the generated manifest, reconstructs each scenario in memory from the embedded seed and recorded mutations, and checks that the reported validation rules include all mutations expected for that scenario.
...
For `generate`, the `--seed` flag is required and selects the valid YAML configuration to mutate. Existing seeds such as `samples/network-with-chaincode.yml` can also be used. Use `--mutation-count` to change the number of mutations per scenario (the default is `3`) and `--output` to select the generated corpus directory (the default is `output`). Generation writes only the Zstandard-compressed scenario manifest to `<output-directory>/scenarios.parquet`; it does not create individual configuration files. The exact seed YAML is embedded once in the Parquet metadata, making the manifest self-contained. Each row has a nested `mutations` list whose entries contain the expected `rule` and zero-based `operatorIndex`, allowing the configuration to be reconstructed deterministically. Legacy manifests without the embedded seed are not accepted. Generation does not run verification or create or update `results.parquet`.

`validate` reads `scenarios.parquet` from `--output` (default `output`) and writes the Zstandard-compressed `<output-directory>/results.parquet`. For every row it clones the embedded seed, replays the recorded mutations, and validates the reconstructed YAML entirely in memory; no temporary configuration files are created. Aggregate counts remain in `metadata.json`. Validation needs neither the original seed nor a mutation count. Each result is `passed` when all expected rules are reported, `partial` when only some are reported, or `failed` when none are reported or the configuration cannot be processed. Missing rules are included in the result. Validation exits with a non-zero status when any scenario is partial or failed. Generation retains its existing corpus overwrite behavior; validation atomically replaces existing results.

Parquet-aware tools can query the files without loading the complete corpus. For example, DuckDB can join scenario definitions to their validation results:

```sql
SELECT s.scenario, s.mutations, r.status, r.missingRules, r.error
FROM read_parquet('output/scenarios.parquet') AS s
JOIN read_parquet('output/results.parquet') AS r USING (scenario);
```
...
After generation completes, its section also records `total` and `mutationOperatorsUsed`; the latter is the total number of operator applications across all scenarios. After validation completes, its section records `total`, `passed`, `partial`, and `failed`. These validation counts are preserved even when partial or failed scenarios make the command fail.
